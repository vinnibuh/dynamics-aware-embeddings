...
def clean_config(config):
    # import ipdb; ipdb.set_trace()
    cleaned_config = copy.deepcopy(vars(config))
...
def write_options(config, location):
    with pathlib.Path(location / "opt.json").open('w') as f:
        cleaned_config = clean_config(config)
        serial_opt = json.dumps(cleaned_config, indent=4, sort_keys=True)
        print(serial_opt)
        f.write(serial_opt)
        f.flush()
